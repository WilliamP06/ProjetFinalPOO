...
class Oiseau(Animal):
    ...
    """
    def __init__(self, p_numero = 0, p_poid = 0.0, p_taille = 0.0, p_longevite = 0, p_diet = "", p_couleur_plumes = "", p_couleur_bec = "",p_longueur_bec = 0.0, p_enclos = ""):
        """
    ...
    def _set_longueur_bec(self, v):
        if v.isnumeric():
            if (int(v) <= 47) and (int(v) >= 1):
                self.__Longueur_bec = int(v)
